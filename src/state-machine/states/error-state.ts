...
export class ErrorState extends BaseState {
  ...
  private async notifyError(): Promise<void> {
    ...
    const timeoutPromise = new Promise<void>(
      (_, reject) => setTimeout(() => reject(new Error("Notify error timeout")), 15000) // 15 seconds instead of 5
    )
  ...
  private updateMetrics(): void {
    ...
    const metrics = {
      ...
      timestamp: Date.now(),
      meetingDuration: this.context.startTime ? Date.now() - this.context.startTime : 0,
      state: this.stateType,
