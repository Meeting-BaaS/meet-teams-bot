...
import type { SpeakerData, StreamingTranscriptionConfig } from "./types"
import { formatError } from "./utils/Logger"
import { PathManager } from "./utils/PathManager"

const DEFAULT_SAMPLE_RATE: number = 16_000
...
export class Streaming {
  ...
  /**
   * Setup streaming transcription if configured
   */
  private async setupStreamingTranscription(): Promise<void> {
    if (!this.transcriptionConfig) {
      return
    }

    try {
      // Use transcription config's sample_rate if specified, otherwise use our sample_rate
      // StreamingTranscription will validate the sample rate against provider requirements
      const transcriptionSampleRate = this.transcriptionConfig.sample_rate || this.sample_rate

      // Ensure we resample to match what transcription expects
      this.sample_rate = transcriptionSampleRate

      this.streamingTranscription = new StreamingTranscription(
        { ...this.transcriptionConfig, sample_rate: transcriptionSampleRate },
        this.botId
      )
      await this.streamingTranscription.start()
      console.log(`🎙️ Streaming transcription started (${this.transcriptionConfig.provider} @ ${transcriptionSampleRate} Hz)`)
    } catch (error) {
      console.error("Failed to setup streaming transcription:", formatError(error))
  ...
  /**
   * Check if transcription is active (for audio routing decisions)
   */
  private get hasTranscription(): boolean {
    return this.streamingTranscription !== null
  }
  ...
  public processMixedAudioChunk(audioChunk: {
    ...
    // Check if we have any audio destination (output WS or transcription)
    const wsReady = this.output_ws && this.output_ws.readyState === WebSocket.OPEN
    const hasAnyDestination = wsReady || this.hasTranscription
    ...
    if (!hasAnyDestination) {
      ...
      if (now - this.lastWsNotReadyLogTime >= this.WS_NOT_READY_LOG_INTERVAL_MS) {
        console.warn(
          "[Streaming] ⚠️ No audio destination available (WS state:",
      ...
      // Trigger reconnection if output URL configured but not connected
      if (this.outputUrl) {
    ...
    try {
      ...
      if (now - this.lastBrowserStatsLogTime > 5000) {
        console.log(`📊 [Streaming] Processed ${this.browserAudioChunksSent} audio chunks`)
  ...
  private processAndSendAudioChunk(audioData: Float32Array): void {
    ...
    // Track chunks processed (for logging)
