...
export class ScreenRecorder extends EventEmitter {
    ...
    private lastAudioDiagAt: number = 0
    private audioDiagCount: number = 0
    ...
    public async startRecording(page: Page): Promise<void> {
        ...
        try {
            ...
            // FFmpeg doesn't read an empty buffer on its first capture — a
            // cold null-sink has zero samples buffered, causing an initial
            // xrun/click at recording start (especially on fresh pods).
            try {
                ...
                await new Promise<void>((resolve, reject) => {
                    primeFfmpeg.on('close', (code, signal) => {
                        ...
                        reject(
                            new Error(
                                `ffmpeg prime exited code=${code} signal=${signal}`,
                            ),
                        )
                    })
            ...
            try {
                ...
                if (hasGrown) {
                    ...
                    if (consecutiveNoGrowthCount >= MAX_CONSECUTIVE_NO_GROWTH) {
                        ...
                        // Dead audio means the browser's stream isn't reaching
                        // the monitored sink (virtual_speaker.monitor). Capture
     ...
     *
     * FFmpeg records from the monitored sink (`VIRTUAL_SPEAKER_MONITOR`). If
     * that file never grows the recording is silent — the browser's audio
     * isn't reaching the monitored sink. Rather than blindly re-routing
     * (which risks masking the real cause), capture the actual state so the
     * failure mode is confirmable from prod logs: the server default sink,
    ...
    private async logAudioPipelineDiagnostics(
        ...
        try {
            ...
            console.warn(
                `⚠️ [audio-diag] Failed to collect PulseAudio state: ${formatError(error)}`,
            )
        }
    }
