...
export async function startToggleProxy(
    ...
    const template = process.env.RESIDENTIAL_PROXY_TEMPLATE
    if (!template) {
        console.log(
            '[ToggleProxy] No RESIDENTIAL_PROXY_TEMPLATE configured, skipping proxy',
        )
        return null
    }
    ...
    const upstreamUrl = template.replaceAll('{SESSION}', session)
    ...
    try {
        server = new Server({
            ...
            prepareRequestFunction: ({ connectionId, request }) => {
                ...
                if (!shouldProxy(target)) {
                    console.log(
                        `[ToggleProxy] DIRECT  → ${target} (not allowlisted)`,
                    )
                    return {}
                }
                if (useUpstream) {
        ...
        //
        // proxy-chain's Server extends EventEmitter but @types/node@14 (pinned in v1)
        // doesn't surface .on() on the subclass — cast through any to invoke the
        // runtime listener. Drop when @types/node is bumped.
        ;(server as any).on(
            'connectionClosed',
            ({
                connectionId,
        ...
        console.log(
            `[ToggleProxy] ✅ Started on ${proxyUrl} (attempt ${retryCount + 1})`,
        )
...
async function logExitIp(upstreamProxyUrl: string): Promise<boolean> {
    try {
        ...
        console.warn(
            `[ToggleProxy] ⚠️  Upstream proxy unreachable (${msg}) — continuing without proxy`,
        )
        return false
    }
}
