...
declare global {
  interface Window {
    htmlCleanerInterval: NodeJS.Timeout
  }
}
...
export class MeetHtmlCleaner {
  ...
  public async start(): Promise<void> {
    ...
    await this.page.evaluate(async (recordingMode) => {
      async function removeInitialShityHtml(mode: string) {
        ...
        try {
          document.querySelectorAll('[data-purpose="non-essential-ui"]').forEach((elem) => {
      ...
      function removeShityHtml(mode: string) {
        ...
        try {
          const icons = Array.from(document.querySelectorAll("i.google-material-icons")).filter(
            (el) => el.textContent?.trim() === "devices"
          )
          icons.forEach((icon) => {
        ...
        try {
          const moodIcons = Array.from(document.querySelectorAll("i.google-material-icons")).filter(
            (el) => el.textContent?.trim() === "mood"
          )
          if (moodIcons.length > 0) {
            const icon = moodIcons[0]
            let currentElement: HTMLElement | null = icon.parentElement
      ...
      function hideVisitorIndicator(): void {
        try {
          ...
          ).filter((el) => el.textContent?.trim() === "domain_disabled")
            ...
            while (currentElement != null) {
              ...
              if (
                ...
                (ariaLabel.toLowerCase().includes("visitor") ||
                  ...
                  ariaLabel.toLowerCase().includes("organisation"))
              ) {
                currentElement.style.opacity = "0"
                break
              }
              // Also check for tooltip content about visitors
              const tooltip = currentElement.querySelector('[role="tooltip"]')
              if (tooltip?.textContent?.toLowerCase().includes("visitor")) {
                currentElement.style.opacity = "0"
                break
              }
              currentElement = currentElement.parentElement
      ...
      window.htmlCleanerInterval = cleanupInterval
  ...
  public async stop(): Promise<void> {
    ...
    await this.page
      .evaluate(() => {
        if (window.htmlCleanerInterval) {
          clearInterval(window.htmlCleanerInterval)
          delete window.htmlCleanerInterval
        }
      })
