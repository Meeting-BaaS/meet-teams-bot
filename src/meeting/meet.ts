...
async function notAcceptedInMeeting(page: Page): Promise<boolean> {
    try {
        ...
        if (result.matched && result.matchedText && result.pattern) {
            ...
            // Distinguish a true "denied entry" from a post-admission kick. Once
            // the bot has been admitted, ScreenRecorder holds a non-zero
            // meetingStartTime (the point the meeting video is cut from). A
            // removal after that is an expected BotRemoved with a usable partial
            // recording — not a BotNotAccepted failure. The same
            // meetingStartTime > 0 check is what ScreenRecorder.finalize uses as
            ...
            if (
                !wasAdmitted &&
                result.matchedText === "You can't join this video call"
            ) {
                GLOBAL.setShouldRetry(true)
                console.log(
