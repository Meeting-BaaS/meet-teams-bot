    ...
    try {
      ...
      while (true) {
        ...
        const gracePeriodExpired =
          !leftWaitingRoomAt || Date.now() - leftWaitingRoomAt >= gracePeriodMs
      ...
      if (GLOBAL.get().entry_message) {
        ...
        await sendEntryMessage(page, GLOBAL.get().entry_message)
        await sleep(100)
      }
...
async function isInMeeting(page: Page): Promise<boolean> {
  try {
    ...
    console.log(
      `✗ Not in meeting: ${networkStatus.userCount} participants, ${result.count} UI indicators${inWaitingRoom ? ", in waiting room" : ""}`
    )
    return false
  } catch (error) {
...
async function sendEntryMessage(page: Page, enterMessage: string): Promise<boolean> {
  ...
  try {
    ...
    await textarea.fill(truncatedEnterMessage)
