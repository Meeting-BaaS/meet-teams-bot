    ...
    await this.page.evaluate(
      ({
        ...
        function getSpeakerFromDocument(timestamp: number): SpeakerData[] {
          ...
          const newInterfaceElements = documentRoot.querySelectorAll('[data-stream-type="Video"]')
            ...
            .map((element, index) => {
              ...
              if (element.hasAttribute("data-cid")) {
                ...
                if (name) {
                  ...
                  const micIcon = element.querySelector('[data-cid="roster-participant-muted"]')
        ...
        function checkElementAndPseudo(el: HTMLElement): boolean {
          ...
          if (
            ...
            const borderOpacity = Number.parseFloat(beforeStyle.opacity)
          ...
          if (
            ...
            const hasVdiFrameClass = el.classList.contains("vdi-frame-occlusion")
            const borderOpacity =
              Number.parseFloat(beforeStyle.opacity) || Number.parseFloat(borderStyle.opacity)
            const borderColor =
              beforeStyle.borderColor || beforeStyle.borderTopColor || borderStyle.borderColor
            return hasVdiFrameClass || (isBlueish(borderColor) && borderOpacity === 1)
          }
        ...
        function isBlueish(color: string): boolean {
          ...
          if (colorLower.startsWith("#")) {
            ...
            if (colorLower.length === 4) {
              const r = Number.parseInt(colorLower[1] + colorLower[1], 16)
              const g = Number.parseInt(colorLower[2] + colorLower[2], 16)
              const b = Number.parseInt(colorLower[3] + colorLower[3], 16)
              rgb = [r, g, b]
            }
            // Handle long hex format (e.g., #ffffff)
            else if (colorLower.length === 7) {
              const r = Number.parseInt(colorLower.slice(1, 3), 16)
              const g = Number.parseInt(colorLower.slice(3, 5), 16)
              const b = Number.parseInt(colorLower.slice(5, 7), 16)
              rgb = [r, g, b]
            }
          } else {
          ...
          try {
            ...
            if (!areMapsEqual(CUR_SPEAKERS, new_speakers)) {
              ...
              currentSpeakersList.forEach((speaker, index) => {
                console.log(`[TEAMS-DEBUG-SPEAKER] Speaker ${index + 1} : ${speaker.isSpeaking}`)
              })
