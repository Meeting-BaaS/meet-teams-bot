...
export function zoomBrowserInterceptionLogic() {
  try {
    ...
    function broadcastSpeakerUpdate(source: "roster" | "audio"): void {
      ...
      // Decoded audio speakers (levels or active-speaker) prove the signal works; the sharer node doesn't.
      if (speaking.size && speaking !== nodeSpeakerIds && !firstSpeakerAt)
        firstSpeakerAt = Date.now()
