...
export class SoundLevelMonitor {
  ...
  public start(): void {
    ...
    this.audioBuffer = []
    console.log("🎵 Sound level monitor started (for automatic leave detection)")
  }
  ...
  public processAudioChunk(audioData: Float32Array): void {
    ...
    if (this.audioBuffer.length >= this.AUDIO_BUFFER_SIZE) {
      ...
      this.processBatchedAudio(batch).catch((error) =>
        console.error("[SoundLevelMonitor] Error processing batched audio:", formatError(error))
      )
    }
  }
  ...
  private async processBatchedAudio(batch: Float32Array[]): Promise<void> {
    ...
    const totalLength = batch.reduce((sum, buffer) => sum + buffer.length, 0)
