...
export class CAPTCHAHandler {
    ...
    private async checkCAPTCHASolutionAccepted(page: Page): Promise<boolean> {
        try {
            ...
            const detection = await CAPTCHADetector.detectCAPTCHA(page)
     ...
     */
    public async refreshCAPTCHA(page: Page): Promise<boolean> {
        try {
            ...
            console.warn('⚠️ CAPTCHA refresh button not found')
            return false
        } catch (error) {
    ...
    private async cleanup(): Promise<void> {
        try {
            await this.solver.terminate()
        } catch (error) {
            console.warn('⚠️ Error during CAPTCHA cleanup:', error)
        }
    }
     ...
     */
    public updateConfig(newConfig: Partial<CAPTCHAConfig>): void {
