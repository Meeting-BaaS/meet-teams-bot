...
# for the one service this repository ships (the meet/teams pools, preprod only).
...
printf '%s\n' "$preprod_context" | grep -Fqx 'build_matrix=["meet-teams-bots"]'
printf '%s\n' "$preprod_context" | grep -Fqx 'deploy_target=meet-teams-bots'
printf '%s\n' "$preprod_context" | grep -Fqx 'should_run=true'

echo "[INFO] Testing manual dispatch context..."
dispatch_context="$(GITHUB_EVENT_NAME=workflow_dispatch INPUT_ENVIRONMENT=preprod INPUT_SERVICE=meet-teams-bots INPUT_IMAGE_TAG=manual-tag .ci/scripts/resolve-pipeline-context.sh)"
printf '%s\n' "$dispatch_context" | grep -Fqx 'image_tag=manual-tag'
printf '%s\n' "$dispatch_context" | grep -Fqx 'deploy_target=meet-teams-bots'
...
if GITHUB_EVENT_NAME=workflow_dispatch INPUT_ENVIRONMENT=prod INPUT_SERVICE=meet-teams-bots .ci/scripts/resolve-pipeline-context.sh >/dev/null 2>&1; then
  echo "[ERROR] Manual prod dispatch should be rejected" >&2
  exit 1
fi

echo "[INFO] Testing build and push dry-run..."
DRY_RUN=true ENVIRON=preprod .ci/scripts/build-and-push.sh --service meet-teams-bots --image-tag dry-run-tag >/dev/null
rm -f .ci/meet-teams-bots.local-image .ci/meet-teams-bots.remote-image
...
deploy_output="$(DRY_RUN=true HOME="$ROOT_DIR/$FAKE_DEPLOYMENT/home" .ci/scripts/deploy.sh --service meet-teams-bots --environment preprod --image-tag dry-run-tag --deployment-dir "$FAKE_DEPLOYMENT/deployment")"
rm -rf "$FAKE_DEPLOYMENT"
printf '%s\n' "$deploy_output" | grep -Fq 'meet-teams-bots-v2 upgrade'
if DRY_RUN=true .ci/scripts/deploy.sh --service meet-teams-bots --environment prod --image-tag dry-run-tag >/dev/null 2>&1; then
